...
int main()
{
    ...
    int num_students, index;              // 학생수, for를 위한 index
    ...
    {
        students[index].sum = students[index].kor + students[index].math;
        students[index].avg = (students[index].kor + students[index].math) / 2;
        printf("%s >> kor:%d math:%d sum:%d avg:%.2lf \n", students[index].name, students[index].kor, students[index].math, students[index].sum, students[index].avg);
    }
